...
def binarizar(df, var_binarias):
    ...
    for i in var_binarias:
        lista_max.append(i + "_" +df[i].value_counts().index[0])
    df = pd.get_dummies(data=df, columns = var_binarias, prefix = var_binarias)
    df.drop(columns = lista_max, inplace=True)
    return df

def reporte_metricas(model, X_test, y_test, title = ""):
...
def get_metrics(y_true, y_pred):
  '''Obtener las métricas de desempeño para la clase 1 de algún modelo
  output:
  ...
  '''
  aux = classification_report(y_true, y_pred, output_dict=True)
  accuracy = aux["accuracy"]
  precision = aux["1"]["precision"]
  recall = aux["1"]["recall"]
